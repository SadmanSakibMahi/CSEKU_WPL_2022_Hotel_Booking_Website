...
if (isset($_GET['fetch_rooms'])) {
    ...
    //count no. of rooms output variable to store room cards
    $count_rooms = 0;
    $output = "";
    ...
    $room_res = select("SELECT * FROM `rooms` WHERE `adult` >= ? AND `children` >= ? AND `status`=? AND `removed`=?", [$adults, $children, 1, 0], 'iiii'); // ORDER BY `id` DESC
